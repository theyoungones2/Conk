...
void DoShellCommand( struct RexxHost *host, char *comline, BPTR fhout )
{
	...
	(rxc->function)( host, (void **) &array, RXIF_INIT, NULL );
	...
	*cargstr = '\0';
	...
	(rxc->function)( host, (void **) &array, RXIF_ACTION, NULL );
	...
	if( rxc->results && array[0]==0 && fhout )
	{
		...
		if( result )
		{
			if( argarray[0] )
			{
				...
				FPrintf( fhout, "%s = %s\n",
					*((char *)argarray[0]) ? (char *)argarray[0] : "RESULT", result );
				FreeVec( result );
				result = NULL;
			}
...
dsc_cleanup:
	...
	if( cargstr ) FreeVec( cargstr );
	if( array ) (rxc->function)( host, (void **) &array, RXIF_FREE, NULL );
	if( argb ) FreeVec( argb );
	FreeArgs( host->rdargs );
}
