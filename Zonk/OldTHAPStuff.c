...
BOOL THAPSaver( struct Chunk *cnk, struct IFFHandle *iff )
{
	...
	LONG numpaths, longzero = 0;
	struct PathSeg *headseg;
...
//
// Creates a new path.
// Optionally creates an HEAD node.
//

struct Path *CreatePath( BOOL headnode )
{
	struct Path *path;
	struct PathSeg *psg;
...
static BOOL WritePathCompiled( struct IFFHandle *iff, struct Path *path )
{
	...
	struct PathSeg *psg;
	ULONG compsiz;
	WORD prevx, prevy, outbuf[3];
	double dist,dx,dy;

	printf( "WritePathCompiled()\n" );
...
//					printf("prevx: %ld\n",(LONG)prevx );
//					printf("prevy: %ld\n",(LONG)prevy );
//					printf("psg->psg_XPos: %ld\n",(LONG)psg->psg_XPos );
//					printf("psg->psg_YPos: %ld\n",(LONG)psg->psg_YPos );
//					printf("psg->psg_Speed: %ld\n",(LONG)psg->psg_Speed );

					dx = (double)( ( psg->psg_XPos - prevx ) << 5 );
					dy = (double)( ( psg->psg_YPos - prevy ) << 5 );
					dist = sqrt( (dx*dx) + (dy*dy) );
					if( dist != 0.0 )
					{
						outbuf[0] = 0 - (WORD)( dist / (double)psg->psg_Speed );
						outbuf[1] = (WORD)(((double)psg->psg_Speed * dx) / dist);
						outbuf[2] = (WORD)(((double)psg->psg_Speed * dy) / dist);
						if( WriteChunkBytes( iff, outbuf, 6 ) != 6 )
							allok = FALSE;
					}
					else
					{
						/* shit. divisionbyzerofuckup... */
						D(bug( "divisionbyzero - pathseg skipped\n" ) );
					}

//					printf( "line %ld,%ld,%ld\n",
//						(LONG)outbuf[0], (LONG)outbuf[1], (LONG)outbuf[2] );
