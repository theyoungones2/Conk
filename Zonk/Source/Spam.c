...
UWORD __asm Spam_Init( VOID )
{
	...
	if( audioport = CreateMsgPort() )
	{
		...
		for( i = 0; i <= 3; i++ )
		{
			audioreq[i] = (struct IOAudio *)CreateIORequest( audioport,
		...
		for( i = 0; i <= 3; i++ )
		{
			repeatreq[i] = (struct IOAudio *)CreateIORequest( audioport,
				sizeof( struct IOAudio ) );
			if( !audioreq[i] )
				status = SPAMERR_MEM;
		}
...
VOID __asm Spam_Die( VOID )
{
	...
	if( audioport )
	{
		...
		for( i = 0; i < 4; i++ )
		{
			DeleteIORequest( (struct IORequest *)audioreq[i] );
			DeleteIORequest( (struct IORequest *)repeatreq[i] );
		}
		DeleteMsgPort( audioport );
		audioport = NULL;
	}
	...
	register __a1 UBYTE *sampledata, register __d0 UWORD chan )
{
	...
	if( play )
	{
		...
		if((spp->spam_Repeats != 0) && spp->spam_RepeatLength)
		{
			...
			repeatreq[chan]->ioa_Cycles = 0; /* (spp->spam_Repeats == -1) ? 0 : spp->spam_Repeats; */
