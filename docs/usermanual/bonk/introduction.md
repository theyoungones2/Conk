...
while the SHIFT key jumps a page at a time. Holding CTRL and then pressing
a direction moves the display to the extreme edge of the map.
...
The Reveal window lists hidden windows. The cycle gadget lets you view by
window or by Map (a Map may have multiple windows). Just select the window
