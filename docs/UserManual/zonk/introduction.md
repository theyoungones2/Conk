...
"Level Type"
	A Game level is a standard Conk level which uses a Display chunk and
ActionLists (ie a level you've made up with Zonk). Anim levels are ones
where an IFF animation is played, then the next level is called. Pic
levels have not been programmed yet, but will be for displaying still
...
indicates the type of file - Conk, IFF ILBM, IFF ANIM, Tracker Module or
unknown. This lets Ponk know what type of file it is dealing with.
...
are initializing. The question marks let you select ActionLists from the
...
the chunk. In this mode, the gadgets displayed are:

"ActionLists"
	A ListView gadget where you can highlight one of the ActionLists in the
chunk. Double-clicking an ActionList has the same effect as highlighting it and
...
"Listing of..."
	The ListView gadget now displays a listing of the Actions in the ActionList.
...
can flick between the pages using the "Next >" and "< Prev" gadgets. When the
