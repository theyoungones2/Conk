...
.BottomOk
	...
	move.w	SS_ScreenToggle(a2),d0
	lsl.w	d0
	lea	OB_WipeSize(a3),a1
	move.w	d3,(a1,d0.w)

	lea	OB_ModSize(a3),a1
	move.w	d2,(a1,d0.w)
	lsl.w	d0
	lea	OB_ScreenOffset1(a3),a1
	move.l	d1,(a1,d0.w)
...
.Exit
	movem.l	(sp)+,a4-a5/d2-d6
	rts
.ClipTop
	add.l	d4,d1				;Move Y Pos down to clip line
	mulu	SS_Planes(a2),d4
	move.w	d3,d5
	lsr.w	#6,d5
	sub.w	d4,d5				;get new Blit size
	ble	.Exit
	lsl.w	#6,d5
	and.w	#%0000000000111111,d3
	or.w	d5,d3				;Store blit size

	move.w	IM_Size(a0),d5
	and.w	#%111111,d5
	sub.w	#1,d5
	lsl.w	d5
	mulu	d5,d4				;Get blit width * lines to clip

	add.l	d4,a4				;Add ^ onto image & mask pntrs
	add.l	d4,a5
...
.ClipLeft
	...
	and.l	#$FFFF,d1
	cmp.l	d1,d4
	bgt	.Exit				;Exit if amount to clip larger than the bob
	lsr.l	#3,d4
	and.w	#$FFFE,d4			;Get # of words to clip
	move.w	d4,d5
	lsr.w	d5

	move.w	d3,d1
	and.w	#%111111,d1			;Yank out width from blitsize
	sub.w	d5,d1
	and.w	#%1111111111000000,d3
	or.w	d1,d3				;Store new width in blitsize

	add.w	d4,d2				;New D modulo
	add.w	d4,d6				;New A modulo
	and.l	#$FFFF,d4

	add.l	d4,a4				;New Image pntr
	add.l	d4,a5				;New Mask pntr
...
WipeBob
	;assumes bltafwm, lwm, con1, custom set
	;inputs		a2.l = Slice struct
	;		a3.l = Bob struct
	...
	move.w	d3,bltamod(a6)		;amod = dmod 
	move.w	d3,bltdmod(a6)

	move.l	SS_LogBase(a2),a1
	add.l	d1,a1
	move.l	a1,bltdpt(a6)			;destination = log screen

	move.l	SS_TriBase(a2),a1
	add.l	d1,a1
	move.l	a1,bltapt(a6)			;apth = Tri screen
