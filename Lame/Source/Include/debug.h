...
#if DEBUGMEM
#define MYALLOCVEC(size,type,label)  MyDebugAllocVec( (size),(type), label )
#define MYFREEVEC(address)  MyDebugFreeVec( (address) )
#else
#define MYALLOCVEC(size,type,label)  MyAllocVec( (size),(type) )
#define MYFREEVEC(address)  MyFreeVec( (address) )
#endif
